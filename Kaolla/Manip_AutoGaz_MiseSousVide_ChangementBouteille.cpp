...
void CManip_AutoGaz::ChangementBouteille()
{
	...
	if(MessageBoxConfirmation("Fermer la bouteille !\t\nPuis appuyer sur OK\t\n", MB_OKCANCEL)==IDCANCEL)
	{
		ToutFermer();
		FermerInstruments();
		return;
	...
	if(MessageBoxConfirmation("Changer la bouteille -> Ouvrir le manomètre\t\nPuis appuyer sur OK\t\n", MB_OKCANCEL)==IDCANCEL)
	{
		ToutFermer();
		FermerInstruments();
		return;
	...
	AfxMessageBox("Fermer le manomètre\t\nOuvrir la bouteille\t\nOuvrir le manomètre à la pression requise\t\n", MB_OK);
	
	AfxMessageBox("Changement de bouteille terminé\t\n", MB_OK);	
