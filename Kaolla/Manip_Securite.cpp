...
{
	if(GenreExperience == MANUEL)
		SecuriteHautePressionManuelle();
	if(GenreExperience == AUTO)
...
DWORD WINAPI CManip::MesureSecuriteHautePression(LPVOID lpParam)
{
	...
	if (!PressionTropHaute)
	{
		...
		CString message;
		message.Format("Attention ! Haute pression trop élevé\t\nVeuillez la mettre en dessous de : %f bar\t\n",
...
void CManip::SecuriteHautePressionAuto()
{
	...
	{
		if(etape_en_cours != MISE_SOUS_VIDE)
		{
...
DWORD WINAPI CManip::MesureSecuriteTemperatureElevee(LPVOID lpParam)
{
	...
	if (!TemperatureElevee)
	{
		...
		CString message;
		message.Format("Attention ! Température du calo trop élevé par rapport à la température d'expérience\t\n");
		AfxMessageBox(message,MB_ICONERROR);
	}
	else
	{
...
DWORD WINAPI CManip::MesureSecuriteTemperatureFaible(LPVOID lpParam)
{
	...
	if (!TemperatureFaible)
	{
		...
		CString message;
		message.Format("Attention ! Température du calo trop faible par rapport à la température d'expérience\t\n");
		AfxMessageBox(message,MB_ICONERROR);
	}
	else
	{
