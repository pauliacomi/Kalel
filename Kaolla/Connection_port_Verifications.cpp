...
void CConnection_port::VerifUnInstrument(int num_instr,int m_nIndex,BOOL m_bKeithleyVoie1,BOOL m_bKeithleyVoie2,int m_nInstrumentKeithleyVoie2,int m_nMensor,int PortInstr)
{
	switch(m_nIndex)
	{
		...
		case INDEX_KEITHLEY:
			...
			verifInstrument[index_verifInstrument].type = APPAREIL_KEITHLEY;
			...
			if(m_bKeithleyVoie1)
				VerifMesure(&CM_Calo,num_instr,KEITHLEY_V1,&StrPbmCalo,&bPbmCalo);
				
			if(m_bKeithleyVoie2)
			{
				if(m_nInstrumentKeithleyVoie2 == LECTURE_BP)
					VerifMesure(&CM_BP,num_instr,KEITHLEY_V2,&StrPbmBP,&bPbmBP);
				else //if(m_nInstrumentKeithleyVoie2 == LECTURE_HP)
					VerifMesure(&CM_HP,num_instr,KEITHLEY_V2,&StrPbmHP,&bPbmHP);
			}
			break;
		case INDEX_MENSOR:
			verifInstrument[index_verifInstrument].numero = num_instr;
			verifInstrument[index_verifInstrument].type = APPAREIL_MENSOR;
			verifInstrument[index_verifInstrument].COM = PortInstr;
			index_verifInstrument++;
			
			if(m_nMensor == LECTURE_BP)
				VerifMesure(&CM_BP,num_instr,MENSOR_VOIE,&StrPbmBP,&bPbmBP);
				
			if(m_nMensor == LECTURE_HP)
