...
string CManip::NomFichier(string extention)
{
	...
	sprintf(nom_fichier_char,"%s",general.chemin.c_str());

	if(!PathIsDirectory(_T(nom_fichier_char)))
	{
		//mettre erreur
		// Pour l'instant : 
		sprintf(nom_fichier_char,"C:/");
	}
	else
	{
		if(general.experimentateur.surnom.c_str() == "")
		{
			sprintf(nom_fichier_char,"%s/Nouveau_Fichier",general.chemin.c_str());
			if(!PathIsDirectory(_T(nom_fichier_char)))
				CreateDirectory(_T(nom_fichier_char),NULL);
		}
		else
		{
			sprintf(nom_fichier_char,"%s/%s",general.chemin.c_str(),general.experimentateur.surnom.c_str());
			if(!PathIsDirectory(_T(nom_fichier_char)))
				CreateDirectory(_T(nom_fichier_char),NULL);
			sprintf(nom_fichier_char,"%s/%s/%s",general.chemin.c_str(),general.experimentateur.surnom.c_str(),
												general.nom_echantillon.c_str());
			if(!PathIsDirectory(_T(nom_fichier_char)))
				CreateDirectory(_T(nom_fichier_char),NULL);
		}
	}
	sprintf(nom_fichier_char,"%s/%s.%s",nom_fichier_char,general.fichier.c_str(),extention.c_str());
...
string CManip::NomFichierEntete(string extention)
{
	...
	sprintf(nom_fichier_entete_char,"%s",general.chemin.c_str());

	if(!PathIsDirectory(_T(nom_fichier_entete_char)))
	{
		//mettre erreur
		// Pour l'instant : 
		sprintf(nom_fichier_entete_char,"C:/");
	}
	else
	{
		if(general.experimentateur.surnom.c_str() == "")
		{
			sprintf(nom_fichier_entete_char,"%s/Nouveau_Fichier",general.chemin.c_str());
			if(!PathIsDirectory(_T(nom_fichier_entete_char)))
				CreateDirectory(_T(nom_fichier_entete_char),NULL);
		}
		else
		{
			sprintf(nom_fichier_entete_char,"%s/%s",general.chemin.c_str(),general.experimentateur.surnom.c_str());
			if(!PathIsDirectory(_T(nom_fichier_entete_char)))
				CreateDirectory(_T(nom_fichier_entete_char),NULL);
			sprintf(nom_fichier_entete_char,"%s/%s/%s",general.chemin.c_str(),general.experimentateur.surnom.c_str(),
												general.nom_echantillon.c_str());
			if(!PathIsDirectory(_T(nom_fichier_entete_char)))
				CreateDirectory(_T(nom_fichier_entete_char),NULL);
		}
	}
	sprintf(nom_fichier_entete_char,"%s/%s(entete).%s",nom_fichier_entete_char,general.fichier.c_str(),extention.c_str());
