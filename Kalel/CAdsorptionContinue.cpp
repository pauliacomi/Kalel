...
IMPLEMENT_DYNAMIC(CAdsorptionContinue, CPropertyPage)

CAdsorptionContinue::CAdsorptionContinue()
	: CPropertyPage(CAdsorptionContinue::IDD)
{
	Reinitialisation();
}
...
void CAdsorptionContinue::DoDataExchange(CDataExchange* pDX)
{
...
BOOL CAdsorptionContinue::OnCommand(WPARAM wParam, LPARAM lParam)
{
	SetModified(TRUE);
	return CPropertyPage::OnCommand(wParam, lParam);
}

BEGIN_MESSAGE_MAP(CAdsorptionContinue, CPropertyPage)
	ON_BN_CLICKED(IDC_CHECK_ADSORPTION_CONTINUE, &CAdsorptionContinue::OnBnClickedCheckAdsorptionContinue)
END_MESSAGE_MAP()


// Gestionnaires de messages de CAdsorptionContinue

BOOL CAdsorptionContinue::OnInitDialog()
{
	CPropertyPage::OnInitDialog();

	//m_SpinTempsEtalonnageDebit.SetRange(20,60);
	...
	//m_SpinTempsEtalonnageVolumeInter.SetRange(20,60);
	...
	//m_SpinTempsEquilibreContinue.SetRange(0,600.0);
	...
	//m_SpinTempsFinalEquilibre.SetRange(0,600.0);
	...
	//m_SpinPressionFinaleAdsorptionContinue.SetRange(0.0,2.0);
...
void CAdsorptionContinue::Reinitialisation()
{
...
void CAdsorptionContinue::OnBnClickedCheckAdsorptionContinue()
{
	UpdateData(TRUE);
	EnableAdsorptionContinue();
	UpdateData(FALSE);
...
void CAdsorptionContinue::EnableAdsorptionContinue(void)
{
...
BOOL CAdsorptionContinue::OnApply()
{
	Enregistrement_donnees_actuelles();
...
void CAdsorptionContinue::OnCancel()
{
	m_bAdsorptionContinue = donnees_actuelles.a_effectuer;
	
	m_fPressionFinaleAdsorptionContinue = donnees_actuelles.pression_finale_adsorption_continue;
	//m_SpinPressionFinaleAdsorptionContinue.SetPos(m_fPressionFinaleAdsorptionContinue);
	
	m_nTempsEquilibreContinue = donnees_actuelles.temps_equilibre_continue;
	//m_SpinTempsEquilibreContinue.SetPos(m_nTempsEquilibreContinue);
	
	m_nTempsEtalonnageDebit = donnees_actuelles.temps_etalonnage_debit;
	//m_SpinTempsEtalonnageDebit.SetPos(m_nTempsEtalonnageDebit);
	
	m_nTempsEtalonnageVolumeInter = donnees_actuelles.temps_etalonnage_volume_inter;
	//m_SpinTempsEtalonnageVolumeInter.SetPos(m_nTempsEtalonnageVolumeInter);
	
	m_nTempsFinalEquilibre = donnees_actuelles.temps_final_equilibre;
	//m_SpinTempsFinalEquilibre.SetPos(m_nTempsFinalEquilibre);

	CPropertyPage::OnCancel();
}

void CAdsorptionContinue::OnOK()
{
	Enregistrement_donnees_actuelles();
...
void CAdsorptionContinue::Enregistrement_donnees_actuelles()
{
	donnees_actuelles.a_effectuer = m_bAdsorptionContinue;
	donnees_actuelles.pression_finale_adsorption_continue = m_fPressionFinaleAdsorptionContinue;
	donnees_actuelles.temps_equilibre_continue = m_nTempsEquilibreContinue;
	donnees_actuelles.temps_etalonnage_debit = m_nTempsEtalonnageDebit;
	donnees_actuelles.temps_etalonnage_volume_inter = m_nTempsEtalonnageVolumeInter;
	donnees_actuelles.temps_final_equilibre = m_nTempsFinalEquilibre;
}
